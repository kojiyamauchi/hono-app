...
### データベース設定
- Prismaスキーマ: [src/prisma/schema.prisma](src/prisma/schema.prisma)
- マイグレーション: `src/prisma/migrations/`
- 生成されたクライアント: `src/generated/prisma/`（このディレクトリは自動生成され、ESLintで無視されます）
- カスタムPrisma設定: [prisma.config.ts](prisma.config.ts) - `src/`配下のカスタムスキーマ/マイグレーションパスを使用
