...
独自認証の `/auth` とSupabase Authの `/supabase-auth` は、単一プロダクトで二つの認証方式を併用することを目的としたものではない。認証基盤を自前で構築する場合と、マネージドサービスへ委譲する場合の設計・実装上の違いを比較するため、両方を実装している。片方への統合・削除はプロジェクトの目的に関わるため、実装前に設計方針を確認すること。
...
- `Controller`: HTTPリクエストとレスポンスの変換を担当する。routesで検証済みの入力とmiddlewareが設定した値を受け取り、Serviceへ引数として渡す。業務ルールを実装しないこと
- `Service`: ユースケースと業務ルールを担当する。原則としてHonoの `Context` に依存せず、Repositoryやshared以下の層を利用して処理結果をControllerへ返すこと。例外として、Cookie入出力を担う `src/shared/auth/services/` のcookieヘルパーは、Controllerから `Context` を受け取って操作する
- `Repository`: データの取得・更新と、必要なtransaction境界を担当する。HTTPリクエスト・レスポンスの関心事を持ち込まないこと
...
詳細な手順（基本原則の全文・smokeの手段・タイミング別の責務・PRへの検証証跡の項目・レビュー時の再実行手順）は、migration検証Skill（[`.claude/skills/migration-verification/SKILL.md`](.claude/skills/migration-verification/SKILL.md)）に集約する。ここでは常に守る共通原則だけを示す。
...
Issueを作成・整理・本文レビューする場合は、詳細手順として [`.claude/skills/issue-management/SKILL.md`](.claude/skills/issue-management/SKILL.md) を参照すること。

- Issue本文の末尾には `## 記載者` を置き、誰が作成・詳細更新した本文か分かるようにすること
- Issueタイトル接頭辞、本文テンプレート、記載者表記、本文レビュー運用、本文メンテナンス方針の詳細はClaude用Issue運用Skillに従うこと
...
レビュー対応の共通原則を以下に示す。コミットメッセージの具体フォーマット、PR本文更新のチェック項目、確認コマンドなどの手順詳細は、レビュー対応Skill（[`.claude/skills/review-response/SKILL.md`](.claude/skills/review-response/SKILL.md)）に集約する。
...
- Claude版（`.claude/skills/`）とCodex版（`.codex/skills/`）のSkillは、方針・確認項目がずれないよう同期する。
...
- PRレビューの詳細な手順は [`.claude/skills/pr-review/SKILL.md`](.claude/skills/pr-review/SKILL.md) を参照すること
...
- migrationを含む変更の実DB検証の詳細手順は [`.claude/skills/migration-verification/SKILL.md`](.claude/skills/migration-verification/SKILL.md) を参照すること
